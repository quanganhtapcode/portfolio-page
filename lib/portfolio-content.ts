...
export const defaultPortfolioContent: PortfolioContent = { settings: defaultSettings, customBlocks: [], research: [
  ...
  { dates: "Jan 2025 ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â Apr 2025", role: "Equity Research Analyst Intern", company: "DSC Securities Corporation", description: "Produced a full equity-research report on KBC and IDC, covering FDI inflows, China+1 manufacturing dynamics, and Global Minimum Tax impacts. Modelled earnings scenarios using lease-rate trends and macro assumptions." },
  { dates: "Jun 2024 ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â Aug 2024", role: "Research & Investment Intern", company: "FPT Capital", description: "Authored VietnamÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¾Ãƒâ€šÃ‚Â¢s 2025 investment outlook and analysed the FUEFCV50 ETFÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¾Ãƒâ€šÃ‚Â¢s structure, index composition, performance attribution, catalysts and risks." },
  { dates: "2022 ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â 2023", role: "Sales Associate", company: "Cat Hai Oil & Grease", description: "Advised industrial and automotive clients, increasing average transaction value by 15% through consultative selling." },
] };
...
function getScale(value: unknown, fallback: number) { return typeof value === "number" && Number.isFinite(value) && value >= 0.4 && value <= 3 ? value : fallback; }
...
function getColor(value: unknown, fallback: string) { return typeof value === "string" && /^#[0-9a-fA-F]{6}$/.test(value) ? value : fallback; }
function getCustomLinks(value: unknown) {
  if (!value || typeof value !== "object" || Array.isArray(value)) return {};
  return Object.fromEntries(Object.entries(value as Record<string, unknown>).flatMap(([key, url]) => {
    if (key.length > 160 || typeof url !== "string" || url.length > 2048) return [];
    try { const parsed = new URL(url); return ["http:", "https:"].includes(parsed.protocol) ? [[key, url.trim()]] : []; } catch { return []; }
  }));
}
