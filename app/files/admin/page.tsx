...
export default function FilesAdminPage() {
  ...
  if (!authenticated) return <main className={styles.page}><header className={styles.header}><a className={styles.brand} href="/">LA<sup>&reg;</sup></a><a className={styles.adminLink} href="/admin/content">Edit portfolio</a><a className={styles.adminLink} href="/files">View library <span className={styles.arrow} aria-hidden="true" /></a></header><section className={styles.login}><p className={styles.eyebrow}>Private area</p><h1>File admin.</h1><p>Sign in to upload, organise and share files.</p><form onSubmit={signIn}><label htmlFor="password">Admin password</label><input id="password" type="password" value={password} onChange={(event) => setPassword(event.target.value)} autoFocus required /><button disabled={busy}>{busy ? "Signing in..." : "Sign in"}</button></form>{message && <p className={styles.message}>{message}</p>}</section></main>;

  const parent = currentFolder.split("/").slice(0, -1).join("/");
  return <main className={styles.page}>
    <header className={styles.header}><a className={styles.brand} href="/">LA<sup>&reg;</sup></a><div className={styles.headerActions}><a className={styles.adminLink} href="/admin/content">Edit portfolio</a><a className={styles.adminLink} href={currentFolder ? `/files?folder=${encodeURIComponent(currentFolder)}` : "/files"}>View folder <span className={styles.arrow} aria-hidden="true" /></a><button className={styles.textButton} onClick={signOut}>Sign out</button></div></header>
