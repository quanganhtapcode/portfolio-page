...
export async function GET(request: Request, { params }: { params: Promise<{ key: string[] }> }) {
  try {
    ...
    return new NextResponse(html, { headers: { "Content-Type": "text/html; charset=utf-8", "Content-Security-Policy": "default-src 'self'; frame-src 'self'; style-src 'unsafe-inline'", "Cache-Control": "no-store" } });
  } catch (error) {
    console.error(error);
